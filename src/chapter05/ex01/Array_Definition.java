 ...
 * 				- 메모리의 stack 공간에 병수명과 참조주소(heap 영역의 주소)
 * 				-변수의 들어갈 값은 heap영역에 저장됨
 * 		변수 - 변수에는 하나의 값만 저장됨
 * 			- 초기값을 할당후 변수를 사용.
 * 		배열 - 하나의 배열 변수에 많은 값을 저장 할수 있다.
 * 			- 배열의 특징 :1. 배열변수의 동일한 자료형의 값만 저장할수 있다(int, long, double)
...
//		변수 선언후 초기값을 할당하지 않고 사용하면 오류발생
		int c;
		int d;
//		int sum = c + d; // 오류발생 : c,d변수에 초기값을 넣지 않고 사용했으므로 오류
		
		System.out.println("===배열 1. 선언 방법===");
		
//		* 배열 선언 1.
		int[] array1 =new int[3]; //배열을 선언과 동시에 방의 크기를 저장
//		int[] : 정수 값만 배열에 저장 가능
//		array : 배열 변수, array1변수명은 stack공간에 저장이되고, array1에 heap영역의 주소값이 저장됨
// 		new : heap영역에 값을 저장하라.
//		int[3] : heap영역에 저장할 방의 개수를 지정
//			index : 방의 번호, 0,1,2

//		배열에 값 할당.
		array1[0] = 55; // array1의 heap영역의 주소 0번째 방의 55를 넣어라
		array1[1] = 66; 
		array1[2] = 77;
//		배열의 값을 출력
		...
		int[] array2; //배열 변수 선언후 
		array2 = new int[5]; //배열 변수의 들어갈 방의 개수 지정
//		array2 배열 변수에 값을 넣고 출력. 
		for(int i =0;i<5;i++) {
			array2[i] = i;
			System.out.println("array2 "+i+"번째방 값 : "+array2[i]);
		}
		System.out.println("===배열 2. 선언 방법===");
//		배열 선언과 방 크기 할당
...
//		System.out.println(array3[4]); 오류 발생 : 실행시 오류 발생
		...
		System.out.println("===배열 : 3. 선언 방법===");
		
		int[] array5 = new int[] {1,2,3,4,5};
//		int[] array6 = new int[5] {1,2,3,4,5,};
//			오류 발생 : 선언과 동시에 배열변수에 값을 할당할때 방크기를 지정하면 오류 발생
...
//		System.out.println(array7[6]);  // 오류 발생
		//ArrayIndexOutOfBoundsException : 배열 방의 크기를 넘어가면 발생되는 오류
		
		System.out.println("===배열 : 4. 선언 방법===");
//		배열 선언과 동시에 값 할당 : 선언과 값 할당 분리시 오류 발생
